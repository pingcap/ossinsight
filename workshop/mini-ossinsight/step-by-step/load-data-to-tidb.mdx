...
* **port**: 4000, by default, tidb cloud use the 4000 port instead of 3306
...
export DATABASE_URL=tidb://<your database username>:<your database password>@<your database host>:4000/gharchive_dev;
docker-compose pull;
docker-compose up;
```
</TabItem>
...
With TiDB Cloud, you can load this data in web console. Because the Developer Tier uses `Shared Ram` and provides 1GB of disk, so we only provide *240k* rows of data for importing.

1. Click `Import` button in cluster detail page
2. Input the config:
  * **Data Source Type**: AWS S3
  * **Bucket URL**: s3://developer.pingcap.com/ossinsight/sample/three-repos/
...
4. The task will be finished after 1~2 minutes, then click `Exit`
...
On top-right of the cluster detail page, click `Connect` and get you connection command like below:
```bash
mysql --connect-timeout 15 -u '(your database username)' -h gateway01.us-west-2.prod.aws.tidbcloud.com -P 4000 -D test -p
```
