...
export function getConnectionOptions(options?: PoolOptions): PoolOptions {
    ...
    return getConnectionOptionsImpl(process.env.DATABASE_URL, options);
}

export function getShadowConnectionOptions(options?: PoolOptions): PoolOptions | null {
    if (process.env.SHADOW_DATABASE_URL === undefined || process.env.SHADOW_DATABASE_URL.length === 0) {
        return null;
    }

    return getConnectionOptionsImpl(process.env.SHADOW_DATABASE_URL, options);
}

function getConnectionOptionsImpl(urlStr: string, options?: PoolOptions): PoolOptions {
