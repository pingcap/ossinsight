...
import { notFalsy } from '@/utils/value';
...
function getAiQuestionHeaders (headers: AxiosResponse['headers']) {
  const limit = String((headers as any)['x-playground-generate-sql-limit'] ?? 'NaN');
  const used = String((headers as any)['x-playground-generate-sql-used'] ?? 'NaN');
  return {
