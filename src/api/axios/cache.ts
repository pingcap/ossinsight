...
const cacheRequest = (cache: Cache) => async (config: InternalAxiosRequestConfig): Promise<InternalAxiosRequestConfig> => {
  const key = makeKey(config);

  if (notNullish(key)) {
    const value = await cache.get(key);
    if (notNullish(value)) {
      config.adapter = async () => await Promise.resolve(value);
    }
  }
  return config;
};
