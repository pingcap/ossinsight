...
export function d3Hierarchy (seriesData: D3HierarchyItem[], maxDepth: number): CustomSeriesOption {
  const displayRoot = buildHierarchy();

  function buildHierarchy (): any {
    return (d3 as any).stratify()
      .parentId(function (d: D3HierarchyItem) {
        return d.parentId;
      })(seriesData)
      .sum(function (d: D3HierarchyItem) {
        return d.value || 0;
      })
  ...
  function overallLayout (params: CustomSeriesRenderItemParams, api: CustomSeriesRenderItemAPI) {
    ...
    displayRoot.descendants().forEach(function (node: any, index: number) {
