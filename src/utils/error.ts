...
import { isFalsy, isNonemptyString, notNullish } from '@/utils/value';
...
export function getErrorMessage (e: unknown): string {
  if (isAxiosError(e)) {
    if (notNullish(e.response)) {
      ...
      const msg = ERROR_MESSAGE_FIELDS.reduce((msg: string | undefined, field) => msg ?? (payload?.[field] as string | undefined), undefined);
...
export function getAxiosErrorPayload (e: AxiosError): unknown {
  if (notNullish(e.response)) {
    const data = e.response.data;
    if (notNullish(data) && typeof data === 'object') {
      const payload = data as { payload?: unknown };
      if (notNullish(payload.payload)) {
