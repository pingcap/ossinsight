import Head from '@/compat/docusaurus/Head';
import { Redirect, useHistory, useLocation } from '@/compat/docusaurus/router';
...
import { isNullish, notNullish } from '@/utils/value';
import { Highlights } from '@/dynamic-pages/analyze/sections/98-Highlights';
import ScrollSpy, { ScrollSpyInstance } from '@/components/LegacyScrollSpy';
import BrowserHash from '@/components/BrowserHash';
import { usePlayground } from '@/dynamic-pages/analyze/playground/Playground';
...
function AnalyzePage () {
  ...
  const onRepoChange = useCallback((repo: Repo | null) => {
    if (!repo) {
      return;
    }
    history.push({
...
export default function AnalyzePageEntry () {
...
function useMainRepo (): AsyncData<InfoPack> & { name: string } {
  const { params: { owner, repo: repoName } } = useRouteMatch() as { params: AnalyzePageParams };
...
function useVsRepo (): AsyncData<InfoPack> & { name: string | undefined, setName: (val: string | undefined) => void } {
  const [vs, setVs] = useUrlSearchState('vs', stringParam()) as [string | undefined, (value: string | undefined) => void];
