...
export const TimeHeatChart = withChart<TimeHeatData, { zone: number }>(({
  ...
  tooltip: itemTooltip({
    ...
    formatter: (params) => {
      ...
      const rawValue = (first as { value?: unknown[] | Record<string, unknown> })?.value;
      let value: TimeHeatData | null = null;
      if (Array.isArray(rawValue)) {
        value = {
          hour: Number(rawValue[0]),
          dayofweek: Number(rawValue[1]),
          pushes: Number(rawValue[2]),
        };
      } else if (rawValue && typeof rawValue === 'object') {
        value = rawValue as TimeHeatData;
      }
      if (!value || Number.isNaN(value.hour) || Number.isNaN(value.dayofweek) || Number.isNaN(value.pushes)) {
        return '';
      }
