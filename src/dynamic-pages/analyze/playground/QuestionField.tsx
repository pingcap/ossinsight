...
import { getOptionalErrorMessage } from '@/utils/error';
import { LoadingButton } from '@mui/lab';
import { isNullish, notFalsy, notNullish } from '@/utils/value';
import { BaseInputBottomLine, BaseInputContainer } from '@/dynamic-pages/analyze/playground/styled';
import PredefinedGroups, { PredefinedGroupsProps } from '@/dynamic-pages/analyze/playground/PredefinedGroups';
import { AiQuestionResource } from '@/api/core';
...
export default function QuestionField ({ defaultQuestion, maxLength, value, loading, error, onAction, onChange, question, onSelectQuestion, resource }: QuestionFieldProps) {
  const handleCustomQuestion: KeyboardEventHandler = useCallback((e) => {
    if (isHotkey('Enter', e.nativeEvent as KeyboardEvent)) {
      e.preventDefault();
      onAction();
    }
  }, [onAction]);
