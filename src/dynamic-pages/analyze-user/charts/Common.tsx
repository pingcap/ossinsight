...
export const Common = ({ hideZoom = false, scrollY }: { hideZoom?: boolean, scrollY?: number }) => {
  ...
  return (
    <>
      ...
      <Grid top={64} left={8} right={scrollY ? 16 : 8} bottom={!hideZoom ? 48 : 8} containLabel />
      <Tooltip trigger="axis" axisPointer={{ type: 'shadow' }} />
      {!hideZoom ? <SliderDataZoom id="x" showDataShadow={false} /> : undefined}
      {scrollY ? <SliderDataZoom id="y" yAxisIndex={0} showDataShadow={false} showDetail={false} maxValueSpan={10}
                                 minValueSpan={scrollY} zoomLock handleStyle={{ opacity: 0 }} width={8} /> : undefined}
    </>
  );
};
