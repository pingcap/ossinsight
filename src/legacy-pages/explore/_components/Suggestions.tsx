...
import QuestionCard, { QuestionCardVariant } from '@/legacy-pages/explore/_components/QuestionCard';
import { generateQuestion, QuestionTemplate } from '@/api/explorer';
import useSWR from 'swr';
import { array } from '@/utils/generate';
...
export function useRecommended (aiGenerated: boolean, n?: number, tagId?: number) {
  const { data, isValidating, mutate } = useSWR([aiGenerated, n, tagId, 'data-explorer-recommend-question'], {
    fetcher: async ([aiGenerated, n]: [boolean, number | undefined, number | undefined, string]) => await generateQuestion(aiGenerated, n, tagId),
