...
import { getQuestionTags, getTags, Question } from '@/api/explorer';
import { useAsyncOperation } from '@/hooks/operation';
...
export default function QuestionSettings () {
  ...
  const { data: questionTags, isValidating: questionTagsLoading, mutate } = useSWR(notNullish(question) ? [question.id, 'question:tags'] : undefined, {
    fetcher: async ([id]: [string, string]) => await getQuestionTags(id),
  });

  const [localTags, setLocalTags] = useState(questionTags ?? []);
  const { run: handleUpdateTags, loading: updatingTags } = useAsyncOperation({ ids: localTags.map((tag: any) => tag.id) }, updateTags, 'admin-settings');
  useEffect(() => {
    setLocalTags((questionTags ?? []).map((tag: any) => tags.find((t: any) => t.id === tag.id) ?? tag));
  }, [questionTags]);
