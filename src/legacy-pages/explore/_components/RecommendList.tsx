import React, { useState } from 'react';
import TagSelector from '@/legacy-pages/explore/_components/TagSelector';
import { Box, GridLegacy as Grid, Typography, useEventCallback } from '@mui/material';
import { QuestionTag } from '@/api/explorer';
import QuestionList from '@/legacy-pages/explore/_components/QuestionList';
import QuestionCard from '@/legacy-pages/explore/_components/QuestionCard';
import Ads from '@/legacy-pages/explore/_components/Ads';

const presetQuestions = [
  {
    question: 'How diverse is django\'s community (by coders\' distribution)',
    questionId: '75404c1b-790f-421c-b530-49b6d336290d',
    imageUrl: require('./img/img1.png').default,
    tags: [{
      id: 2,
      label: 'Repositories',
      color: '#a3fcc8',
      sort: 2,
      createdAt: '2023-01-19T02:36:34.000+00:00',
    }],
  },
  {
    question: 'Summary of @gvanrossum\'s contribution by event type in 2022',
    questionId: '8ff46822-4637-4632-8306-81e3e28333d2',
    imageUrl: require('./img/img2.png').default,
    tags: [{
      id: 1,
      label: 'Developers',
      color: '#f2ac99',
      sort: 1,
      createdAt: '2023-01-19T02:36:34.000+00:00',
    }],
  },
];

export default function RecommendList () {
  const [tag, setTag] = useState<QuestionTag | null>(null);

  const handleTagChange = useEventCallback((_, tag: QuestionTag | null) => {
    setTag(tag);
  });

  return (
    <Grid container spacing={4}>
      <Grid item xs={12} md={5} lg={4}>
        <Typography variant="h3" fontSize={16} mb={2}>
          💡 Popular questions
        </Typography>
        {presetQuestions.map((props, index) => <QuestionCard key={index} {...props} />)}
        <Ads sx={{ mt: 2 }} utmContent='home_left' />
      </Grid>
      <Grid item xs={12} md={7} lg={8}>
        <TagSelector value={tag} onChange={handleTagChange} />
        <Box>
          <QuestionList n={100} tagId={tag?.id ?? undefined} />
        </Box>
      </Grid>
    </Grid>
  );
}
