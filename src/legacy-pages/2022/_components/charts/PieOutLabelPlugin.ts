import { ArcElement, ArcProps, Chart, ChartConfiguration, ChartDataset, ChartMeta, Color, PieController, Plugin } from 'chart.js/auto';
    ...
    chart.data.datasets
      .forEach((dataset, i) => {
        if (isPieLike(chart, dataset)) {
          // Start here
          const meta: ChartMeta<ArcElement> = chart.getDatasetMeta(i) as any;
          const max = Math.max(...dataset.data as number[]);
          meta.data.forEach((datapoint: ArcElement, i: number) => {
      ...
      .forEach((dataset, i) => {
        if (isPieLike(chart, dataset)) {
          ...
          meta.data.forEach((datapoint: ArcElement, i: number) => {
            ...
            const c = { x: Number(x ?? 0), y: Number(y ?? 0) };
            const angle = (Number(startAngle ?? 0) + Number(endAngle ?? 0)) / 2;
            const point = getPointOnArc(c, Number(outerRadius ?? 0), angle);
