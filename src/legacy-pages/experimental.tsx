...
import { STORAGE_KEY, useExperimental } from '@/components/Experimental';
import NotFound from '@/compat/theme/NotFound';
...
function ExperimentalSwitch ({ feature }: { feature: string }) {
  ...
  return (
    <Switch checked={enabled} onChange={useCallback((_: unknown, checked: boolean) => enable(checked), [enable])}></Switch>
  );
}
