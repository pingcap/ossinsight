...
const languageColors = LANGUAGE_COLORS as Record<string, string>;

for (const lang in languageColors) {
  languageColors[lang.toLowerCase()] = languageColors[lang];
}
...
export default function TopListV2PagePlaceholder () {
  return null;
}
...
const renderData = (data: ProcessedTopListData[], offset: number, history: { push: (path: string) => void }) => {
  return data.map((item, i) => (
    <TableRow key={item.repo_id}>
      ...
      <TableCell>
        ...
        {item.language && (
          <RepoMeta>
            <Dot sx={{ backgroundColor: languageColors[item.language?.toLowerCase()] ?? '#d1d1d1' }} />
            {item.language}
          </RepoMeta>
        )}
      </TableCell>
...
const renderCollections = (names: string[] | undefined | null, history: { push: (path: string) => void }) => {
  return (
