...
export default function WorldMapChart<T> (props: WorldMapChartProps<T>) {
  ...
  const options: EChartsOption = useMemo(() => {
    ...
    if (isNullish(compareData)) {
      series = data.map((item): ScatterSeriesOption | EffectScatterSeriesOption => {
        const title = alpha2ToTitle(item[dimensionColumnName] as string) ?? String(item[dimensionColumnName]);
        const value = Number(item[metricColumnName] ?? 0);
        const { long = 0, lat = 0 } = alpha2ToGeo((item[dimensionColumnName] as string).toUpperCase()) ?? {};
      ...
      series = [data, compareData].map((data, i): ScatterSeriesOption | EffectScatterSeriesOption => ({
        type: effect ? 'effectScatter' : 'scatter',
        geoIndex: 0,
        coordinateSystem: 'geo',
