import { useHistory } from '@/compat/docusaurus/router';
...
import { notNullish, isFalsy } from '@/utils/value';
import { cleanGitHubUrl } from '@/utils/github-url';
...
const isRepoOption = (option: Option): option is SearchRepoInfo => 'fullName' in option;
const isUserOrOrgOption = (option: Option): option is UserInfo | SearchOrgInfo => 'login' in option;
...
const GeneralSearch: FC<GeneralSearchProps> = ({ contrast, align = 'left', size, global = false }: GeneralSearchProps) => {
  ...
  const handleOptionChange = useCallback((_: any, option: Option | null) => {
    if (isFalsy(option)) {
      return;
    }
    setOption(option);
  ...
  const handleKeyDown: KeyboardEventHandler<HTMLInputElement> = useEventCallback((e) => {
    if (isHotkey(['left', 'right', 'tab'], e.nativeEvent)) {
      e.preventDefault();
      next();
    }
  });
    ...
    <Autocomplete<Option, false, undefined, false>
      ...
      renderOption={useCallback(
        (props: any, option: Option) => {
          if (isUserOrOrgOption(option)) {
            return renderUser(props, option, highlight === option);
          }
          if (isRepoOption(option)) {
