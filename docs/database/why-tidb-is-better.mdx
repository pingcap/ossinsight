...
import {useGroups} from "../../src/api/groups";
import GroupSelect from "../../src/components/GroupSelect";
const groups = osdbgroup.reduce((p, c) => { p[c.group_name] = { name: c.group_name, repoIds: c.repos.map(item => item.id)}; return p }, {})
const tidbGroup = groups.tidb.repoIds
