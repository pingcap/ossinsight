...
Judging from the total number of issues, TiDB repo has a total of 155562 issues, ranking the top 3 in all open source database repos, and 4675 more than clickhouse. We judge the activeness of an open source repository by referring to the number of issues. Generally, the higher number of issues means more developers are involved in a repository, which presents the active level of it. Therefore, the activity of the TiDB repo can be considered to be at the forefront of the open source database area.
...
We can see that in addition to China, contributors from 38 countries around the world have submitted issues for the TiDB repository from this .
```
issue-creators-map
```
...
From the perspective of geographical distribution, people who contributed to TiDB can also 
measure the repository's popularity and diversity in all open source database repos. We can see that TiDB is an active, inclusive, and international open source project in this map.
...
It is unilateral to prove that the repository is active only by the number of submitted PRs. It is also necessary to pay attention to the life cycle of a PR (from start to finish) to assist in analyzing the activity and health of open source repos. As you can see from the bar chart below, the PR life cycle length varies widely between repos. On average, hive and ignite take almost 120 days to close a PR, while TiDB, which is also on the list, only takes about 10 days.
