...
|------|-------|----------|-----------|------------|
| [CLI-Anything](https://ossinsight.io/analyze/HKUDS/CLI-Anything) | 25,212 | 23 | **1,096** | 8.96% |
| [agent-browser](https://ossinsight.io/analyze/vercel-labs/agent-browser) | 25,808 | 79 | 331 | 6.06% |
| [Google Workspace CLI](https://ossinsight.io/analyze/googleworkspace/cli) | 23,219 | 28 | **829** | 4.90% |
| [jackwener/opencli](https://ossinsight.io/analyze/jackwener/opencli) | 9,231 | 16 | **576** | 8.35% |
| [Agent-Reach](https://ossinsight.io/analyze/Panniantong/Agent-Reach) | 13,881 | 34 | 408 | 7.90% |
| [larksuite/cli](https://ossinsight.io/analyze/larksuite/cli) | 4,722 | 5 | **944** | 4.62% |

*Data from GitHub API, March 31, 2026*
...
**[Agent-Reach](https://ossinsight.io/analyze/Panniantong/Agent-Reach)**: "Give your AI agent eyes to see the entire internet. Read & search Twitter, Reddit, YouTube, GitHub, Bilibili, XiaoHongShu — one CLI, zero API fees."
...
I cloned CLI-Anything to see what the actual implementation philosophy is. The HARNESS.md file (their methodology guide) reveals a design decision most coverage misses:

The harnesses don't reinvent the software — they **expose the existing backend**:

> *"Most GUI apps separate presentation from logic. Find the core library/framework (e.g., MLT for Shotcut, ImageMagick for GIMP). Map GUI actions to API calls — every button click corresponds to a function call."*

For Blender: the harness calls `blender --background` with Python scripts. For LibreOffice: it calls `libreoffice --headless`. For Kdenlive: it calls `melt` (the MLT framework CLI). The CLI isn't an emulation layer — it's a thin wrapper around the backends that already exist.

This is architecturally significant. It means the harnesses are **stable** (they inherit the stability of the underlying backends) and **composable** (standard Python Click CLIs that behave consistently).

Every generated CLI ships with three things:
1. A `--json` output flag for machine-readable results
2. A REPL mode for stateful multi-step operations  
3. A `SKILL.md` autodiscovery file so AI agents can learn the interface

The skill file is the clever part. It's not documentation for humans — it's a structured capability declaration for agents:

```yaml
# From CLI-Anything's openclaw-skill/SKILL.md
name: cli-anything
description: Use when the user wants OpenClaw to build, refine, test, 
  or validate a CLI-Anything harness for a GUI application.
```

By standardizing this format, CLI-Anything created an **interface registry** — not just for its own CLIs, but as a template for the entire ecosystem.
