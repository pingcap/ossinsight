...
Skills today are often overlapping, hard to standardize, and [sometimes unsafe to reuse directly](https://arxiv.org/abs/2601.10338). Research suggests significant duplication across skill repos and real security concerns in roughly a quarter of publicly shared skills. The abstraction is leaky.
...
If skills are only a transitional layer, then what sits around them?

Below: **tools, data, memory** — the infrastructure a skill assumes but doesn't contain.
Above: **planning, reasoning, orchestration** — the judgment layer that decides *which* skill to use and *when*.

Skills are the middle. Strip away either end, and they stop working. It's like distributing a recipe without mentioning you need a kitchen — or a cook.
