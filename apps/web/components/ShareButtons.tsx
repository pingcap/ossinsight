...
export default function ShareButtons({ url, title, className, stars, forks, language, hashtags }: ShareButtonsProps) {
  ...
  return (
    <div className={`fixed right-3 top-1/2 z-50 hidden -translate-y-1/2 flex-col items-center gap-1 rounded-lg bg-[#1a1a1b]/80 p-1.5 shadow-lg backdrop-blur xl:flex ${className ?? ''}`}>
      {/* Native share (mobile) */}
      {canNativeShare && (
        <button
          onClick={nativeShare}
