...
export function registerEtlTasks(scheduler: Orbital): void {
  // General ETL process
  scheduler.define('etl.process', async (job) => {
    const { pipelineId, source, destination, options = {} } = job.data as EtlProcessData;
  ...
  scheduler.define('etl.transform', async (job) => {
    const { transformId, input, output, transformType } = job.data as EtlTransformData;
  ...
  scheduler.define('etl.load', async (job) => {
    const { loadId, data, target, mode } = job.data as EtlLoadData;
...
async function loadDataToTarget(
  loadId: string,
  data: unknown[],
  target: string,
  mode: string
...
async function processDailyEtlJobs(): Promise<void> {
