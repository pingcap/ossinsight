SELECT
    TRIM(REPLACE(REPLACE(REPLACE(LOWER(gu.organization), '@', ''), ',', ''), ' inc.', '')) AS company,
    COUNT(*) AS contributions
FROM github_events ge
JOIN github_users gu ON ge.actor_login = gu.login
JOIN github_repos gr ON ge.repo_id = gr.repo_id
WHERE
    ge.created_at BETWEEN '2021-01-01' AND '2022-01-01'
    AND (
