WITH prs_with_merged_at AS (
    ...
    WHERE
        ...
        AND actor_login NOT LIKE '%bot%'
        AND actor_login NOT IN ('cockroach-teamcity', 'elasticmachine', 'ti-srebot', 'sre-bot', 'ti-chi-bot')
), tdiff AS (
    SELECT
        pwo.repo_id,
        TIMESTAMPDIFF(HOUR, pwo.opened_at, pwm.merged_at) / 24 AS diff,
        ROW_NUMBER() OVER (PARTITION BY pwo.repo_id ORDER BY TIMESTAMPDIFF(HOUR, pwo.opened_at, pwm.merged_at)) AS r,
        COUNT(*) OVER (PARTITION BY pwo.repo_id) AS cnt
    FROM prs_with_opened_at pwo
    JOIN prs_with_merged_at pwm ON (pwo.repo_id = pwm.repo_id AND pwo.number = pwm.number)
)
SELECT
    gr.repo_id,
    gr.repo_name,
