WITH issues_with_first_responed_at AS (
    SELECT
        repo_id,
        number,
        MIN(created_at) AS first_responed_at
    ...
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY iwo.repo_id ORDER BY (UNIX_TIMESTAMP(iwfr.first_responed_at) - UNIX_TIMESTAMP(iwo.opened_at))) AS r,
        COUNT(*) OVER (PARTITION BY iwo.repo_id) AS cnt
    FROM issues_with_opened_at iwo
    JOIN issues_with_first_responed_at iwfr ON (iwo.repo_id = iwfr.repo_id AND iwo.number = iwfr.number)
)
SELECT
    gr.repo_id,
    gr.repo_name,
